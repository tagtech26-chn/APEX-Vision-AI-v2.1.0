...
export default function App() {
  ...
  const [message, setMessage] = useState(""), [previewFullscreen, setPreviewFullscreen] = useState(false);
  ...
  const render = useCallback(async () => {
    ...
    try {
      const res = await fetch(`${API}/api/render`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ room: room.id, tile: selectedTile.id, tile_size: tileSize, grout_width: groutWidth, grout_color: hexToRgb(groutColor), pattern, material_profile: profile }) });
      ...
      for (let i = 0; i < 120 && current.status !== "done" && current.status !== "error" && current.status !== "superseded"; i++) {
        ...
        const statusRes = await fetch(`${API}/api/render/${initial.job_id}`); if (!statusRes.ok) throw new Error(`Render status failed (${statusRes.status})`);
        current = await statusRes.json(); if (version === renderVersion.current) setJob(current);
      }
      if (version !== renderVersion.current) return;
      if (current.status === "done" && current.image) { setRendered(asset(current.image)); notify(`Render complete in ${current.duration_seconds ?? ""} seconds.`); }
      else if (current.status !== "done") notify(current.message || "Render did not complete.");
    } catch (e) {
      if (version !== renderVersion.current) return;
      setJob({ job_id: "", status: "error", message: e instanceof Error ? e.message : "Render failed" }); notify(e instanceof Error ? e.message : "Render failed");
    }
  }, [room, selectedTile, tileSize, groutWidth, groutColor, pattern, profile, notify]);
  ...
  useEffect(() => {
    ...
    renderTimer.current = window.setTimeout(() => { void render(); }, 350);
    return () => { if (renderTimer.current) window.clearTimeout(renderTimer.current); };
  }, [roomId, tileId, tileSize, groutWidth, groutColor, pattern, profile, render]);

  useEffect(() => { setRendered(null); setJob(null); setShowBefore(false); }, [roomId]);
  ...
  function newProject() { setRoomId(rooms[0]?.id ?? ""); setTileId(tiles[0]?.id ?? ""); setRendered(null); setJob(null); setShowBefore(false); setTileSize(600); setGroutWidth(2); setGroutColor("#dcdcdc"); setPattern("Straight"); setProfile("generic"); notify("New visualizer project started."); }
    ...
    <main className="workspace">
      ...
      <div className={`content-grid ${previewFullscreen ? "full-preview-grid" : ""}`}>
        <section className="visual-area"><div className="original-label"><span>{showBefore ? "Original Image" : rendered ? "AI Rendered Image" : "Original Image"}</span><span>Detection: {surface} <em>✓ Confidence: 0.96</em></span></div><div className={`preview-frame ${viewMode === "Material Only" ? "material-only" : ""}`}>
          ...
          <div className="preview-controls"><button title="Toggle before/after" onClick={() => rendered && setShowBefore(v => !v)} disabled={!rendered}><Compare /></button><button title="Show original" onClick={() => setShowBefore(true)}><ImageIcon /></button><button title={previewFullscreen ? "Exit full window" : "Full window preview"} onClick={() => setPreviewFullscreen(v => !v)}>{previewFullscreen ? <FullscreenExit /> : <Fullscreen />}</button><small>{rendered ? (showBefore ? "Before" : "After") : "Original"}</small></div>
        </div>
        {!previewFullscreen && <><div className="catalog-head"><h2>Material Catalog</h2><div className="search"><input value={query} onChange={e => setQuery(e.target.value)} placeholder="Search materials..." /><Search fontSize="small" /></div></div><div className="chips">{categories.map(c => <button key={c} className={category === c ? "selected" : ""} onClick={() => setCategory(c)}>{c}</button>)}</div><div className="tile-grid">{visibleTiles.map(t => <button key={t.id} className={`tile-card ${tileId === t.id ? "chosen" : ""}`} onClick={() => changeConfig(() => setTileId(t.id))}><img src={asset(t.imageUrl)} /><div><b>{t.name}</b><span>{t.category}</span></div></button>)}</div></>}
        </section>
        {!previewFullscreen && <aside className="right-panel"><div className="panel-tabs"><button className={tab === "scene" ? "on" : ""} onClick={() => setTab("scene")}>Scene</button><button className={tab === "ai" ? "on" : ""} onClick={() => setTab("ai")}>AI Analysis</button></div>{tab === "scene" ? <><Section title="Surface Detection"><label>Detected Surface</label><select value={surface} onChange={e => changeConfig(() => setSurface(e.target.value))}><option>Floor</option><option>Wall</option><option>Ceiling</option></select><label>Surface Area</label><div className="area">28.45 m²</div></Section><Section title="Visualization Mode"><div className="seg"><button className={viewMode === "Realistic" ? "on" : ""} onClick={() => setViewMode("Realistic")}>Realistic</button><button className={viewMode === "Material Only" ? "on" : ""} onClick={() => setViewMode("Material Only")}>Material Only</button></div></Section><Section title="Environment"><div className="seg"><button className={environment === "Interior" ? "on" : ""} onClick={() => setEnvironment("Interior")}>Interior</button><button className={environment === "Exterior" ? "on" : ""} onClick={() => setEnvironment("Exterior")}>Exterior</button></div></Section><Section title="Options"><Toggle label="Smart Removal" value={smartRemoval} onChange={v => changeConfig(() => setSmartRemoval(v))} /><Toggle label="Furniture Shadow" value={shadow} onChange={v => changeConfig(() => setShadow(v))} /><Toggle label="Enhance Lighting" value={lighting} onChange={v => changeConfig(() => setLighting(v))} /></Section><div className="render-settings"><label>Material Profile</label><select value={profile} onChange={e => changeConfig(() => setProfile(e.target.value))}><option value="generic">Generic</option><option value="ceramic">Ceramic</option><option value="stone">Stone</option><option value="wood">Wood</option><option value="vinyl">Vinyl</option><option value="carpet">Carpet</option></select><div className="settings-tabs"><button className="on">Tile</button><button>Grout</button></div><label>Tile Size</label><select value={tileSize} onChange={e => changeConfig(() => setTileSize(Number(e.target.value)))}>{TILE_SIZES.map(size => <option key={size} value={size}>{size} × {size} mm</option>)}</select><label>Grout Width</label><select value={groutWidth} onChange={e => changeConfig(() => setGroutWidth(Number(e.target.value)))}>{GROUT_WIDTHS.map(width => <option key={width} value={width}>{width} mm</option>)}</select><label>Grout Color</label><div className="color-row"><input type="color" value={groutColor} onChange={e => changeConfig(() => setGroutColor(e.target.value))} /><span>{groutColor.toUpperCase()}</span></div><label>Tile Layout</label><div className="pattern-grid">{PATTERNS.map(p => <button key={p} className={pattern === p ? "selected" : ""} onClick={() => changeConfig(() => setPattern(p))}>{patternIcon(p)}<span>{p}</span></button>)}</div></div><div className="auto-render-note"><AutoAwesome fontSize="small" /> Auto Heavy-AI rendering enabled</div></> : <div className="ai-panel"><Insights /><h3>Heavy AI Analysis</h3><p>GroundingDINO + SAM2 + Depth Anything V2</p><div className="ai-ok">● Heavy stack ready</div><p className="muted">Floor detection and object-aware segmentation are active.</p></div>}</aside>}
      </div>
