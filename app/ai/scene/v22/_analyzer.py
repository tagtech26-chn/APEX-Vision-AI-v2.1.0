...
class V22SceneAnalyzer(SceneAnalyzer):
    ...
    def _normal_consistency_mask(
        ...
        cosine = np.sum(
            normals.astype(np.float32) * target[None, None, :], axis=2
        )
        positive = cosine >= minimum_cosine
        negative = cosine <= -minimum_cosine
        return (
            negative
            if int((negative & valid).sum()) > int((positive & valid).sum())
            else positive
        )
    ...
    def _refine_floor_mask(
        ...
        if normals is not None and normals.shape[:2] == seed.shape[:2]:
            refined &= V22SceneAnalyzer._normal_consistency_mask(
                normals, refined
            )
        result = refined.astype(np.uint8) * 255
        result = cv2.morphologyEx(
            result, cv2.MORPH_CLOSE, np.ones((5, 5), np.uint8)
        )
        result = cv2.morphologyEx(
    ...
    def analyze(self, image_path, progress_cb=None):
        ...
        if original_area >= 500 and refined_area < max(
            250,
            int(original_area * 0.12),
        ):
            logger.warning(
        ...
        scene.metadata["v22_geometry"] = {
            ...
            "refined_area_ratio": (
                refined_area / original_area if original_area else 0.0
            ),
