...
class HeuristicDetector(ObjectDetector):
    ...
    def _detect_obstructions(cls, image: np.ndarray) -> list[Detection]:
        """Find sizeable foreground regions without requiring them in the floor mask."""
        h, w = image.shape[:2]
        if h < 20 or w < 20:
            return []

        floor = estimate_floor_mask(image)
        floor_rows = np.where((floor > 0).any(axis=1))[0]
        mask_top = int(floor_rows.min()) if len(floor_rows) else h
        ...
        seed_y0 = min(max(int(h * 0.86), floor_top + int(h * 0.20)), h - 2)
        ...
        texture_threshold = max(
            22.0, float(np.percentile(local_std[seed_y0:, :], 90)) * 1.6
        )
