...
class RenderService:
    ...
    def _cache_key(self, room_path: Path) -> str:
        """Build a cache key that changes when the AI geometry pipeline changes.

        The previous v7 key could reuse a scene generated by an older geometry
        pipeline. V2.2 combines provider identity and an explicit geometry
        pipeline version so development changes cannot silently inherit stale
        floor masks, depth geometry, normals, or homographies.
        """
        analyzer = self.get_analyzer()
        providers = getattr(analyzer, "providers", {}) or {}
        detector = str(providers.get("detector", "unknown"))
        segmenter = str(providers.get("segmenter", getattr(analyzer, "segmenter", "unknown")))
        depth = str(providers.get("depth", getattr(analyzer, "depth", "unknown")))
        safe = lambda value: "".join(ch if ch.isalnum() or ch in "._-" else "-" for ch in value)
        return (
            f"{safe(room_path.stem)}__{safe(detector)}__{safe(segmenter)}__"
            f"{safe(depth)}__geometry-v22"
        )
    ...
    def _load_or_build_scene(self, room_path: Path, progress_cb=None) -> SceneResult:
        ...
        if not self.cache.enabled:
            ...
            try:
                ...
                cached_pipeline = scene.metadata.get("geometry_pipeline_version")
                if cached_fp == fingerprint and cached_pipeline == "v22":
                    ...
                    return scene
                render_metrics.cache_miss()
                logger.info("[CACHE] Stale scene for %s, re-analysing", room_path.name)
        ...
        scene.metadata["geometry_pipeline_version"] = "v22"
